...
- **Search** — type a name (try `Joe's`, `Oita`, `Mazzat`, `Golden`, `New Spot`)
  and press Enter or tap **Look up grade**. Renders matching result cards.
...
- **States covered** — empty guard, single/multi match, no-record, A/B/C grades,
  pending (no letter grade yet), and the Maps-link error.
