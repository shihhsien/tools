...
Dark, flat, mobile-first civic utility. Near-black page (`#0b0b0c`), cards one step lighter (`#161617`) with a 1px hairline border and 12–14px radius, no shadows. System UI font at small sizes; weight 600 carries emphasis, 700 only on the grade letter. The grade palette is the **authentic DOHMH window-card palette**: A blue (`#1f57a6`), B green (`#2f8b4e`), C orange (`#e07d2a`), neutral grey for ungraded — never traffic-light red. For single results, the hero is the white DOHMH placard card (the iconic window-card with the giant grade letter), rendered in the grade ink color. For multi-result lists, use the compact grade chip. One white inverted button per screen with an opacity-dim press state. No imagery, no gradients, no emoji; the only glyph is `↗` on outbound links. Copy is calm and factual, owns its limits, and always offers the next step.
